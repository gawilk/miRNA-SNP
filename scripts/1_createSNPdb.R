...
geneIDs <- sapply(strsplit(SNPdb$gene_assoc, " // "), function(X) {
	c(X[1], X[5])
})
SNPdb$gene <- geneIDs[2, ]
SNPdb$ensembl <- geneIDs[1, ]
...
SNPdb.df <- subset(SNPdb, select = -c(cnv, gene_assoc))
